...
function LessonDetailModal({ lesson, onClose, onComplete, completing }: LessonDetailModalProps) {
 ...
 <div className="bg-primary rounded-3xl border border-subtle max-w-2xl w-full max-h-[85vh] overflow-y-auto flex flex-col animate-scale-up">
 {/* Header */}
 <div className="p-6 border-b border-subtle flex items-start justify-between gap-4">
 <div>
 <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border bg-secondary text-primary border-subtle">
 {lesson.isAI ? 'AI Personalized' : 'Curriculum Module'}
 </span>
 <h2 className="text-xl font-bold text-white mt-2">{lesson.title}</h2>
 </div>
 <button onClick={onClose} className="p-2 rounded-xl hover:bg-secondary text-muted hover:text-white transition-colors">
 <X className="w-5 h-5" />
 </button>
 </div>
 ...
 {/* Why received */}
 <div>
 <h3 className="text-xs font-bold text-muted uppercase tracking-wider mb-2">Why you received this</h3>
 <p className="text-sm text-secondary leading-relaxed bg-secondary border border-subtle rounded-2xl p-4">
 {lesson.why_received}
 </p>
 </div>
 ...
 {/* Behavioral target */}
 <div>
 <h3 className="text-xs font-bold text-muted uppercase tracking-wider mb-2">Behavioral Weakness Explanation</h3>
 ...
 {/* AI Coaching */}
 <div>
 <h3 className="text-xs font-bold text-muted uppercase tracking-wider mb-2">AI-Generated Coaching</h3>
 <p className="text-sm text-primary bg-secondary border border-subtle rounded-2xl p-4 leading-relaxed italic">
 &quot;{lesson.ai_coaching}&quot;
 </p>
 </div>
 ...
 {/* Psychology */}
 <div>
 <h3 className="text-xs font-bold text-muted uppercase tracking-wider mb-2">Driving Psychology Explanation</h3>
 ...
 {/* Exercises */}
 <div>
 <h3 className="text-xs font-bold text-muted uppercase tracking-wider mb-3">Recommended Exercises</h3>
 <ul className="space-y-2.5">
 {lesson.exercises.map((ex, i) => (
 <li key={i} className="flex items-start gap-3 text-sm text-secondary">
 <span className="w-5 h-5 rounded-full bg-brand-500/20 text-brand-400 flex items-center justify-center text-[10px] font-bold shrink-0 mt-0.5">
 {i + 1}
 </span>
 <span>{ex}</span>
 </li>
 ...
 <div>
 <h3 className="text-xs font-bold text-muted uppercase tracking-wider mb-2">Simulation Drills</h3>
 <div className="flex flex-wrap gap-2">
 {lesson.drills.map((mode, i) => (
 <span key={i} className="text-xs bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-xl px-3.5 py-1.5 font-semibold">
 {mode}
 </span>
 ))}
 </div>
 ...
 {/* Improvement Goal */}
 <div className="bg-brand-500/10 border border-brand-500/20 rounded-2xl p-4 flex items-center gap-4">
 <TrendingUp className="w-6 h-6 text-brand-400 shrink-0" />
 <div>
 <p className="text-[10px] font-bold text-brand-500 uppercase tracking-wider mb-0.5">Estimated Improvement Goal</p>
 <p className="text-sm font-bold text-white">{lesson.improvement_goal}</p>
 </div>
 </div>
 </div>

 {/* Footer */}
 <div className="p-6 border-t border-subtle bg-secondary flex items-center justify-between gap-4">
 <div className="text-xs text-muted">
 Difficulty: <strong className="text-white">{lesson.difficulty}</strong>
 </div>
 ...
 onClick={onComplete}
 disabled={completing}
 className="flex items-center gap-1.5 bg-brand-600 hover:bg-brand-500 text-white text-sm font-bold px-5 py-2.5 rounded-xl transition-colors disabled:opacity-50"
 >
 {completing ? (
...
function AILessonCard({ lesson, index, onOpen }: { lesson: AILesson; index: number; onOpen: (l: AILesson) => void }) {
 ...
 {/* Behavioral target */}
 <p className="text-sm text-secondary leading-relaxed font-medium mt-1">{lesson.behavioral_target}</p>
 ...
 <span className="text-[11px] font-bold uppercase tracking-wider text-primary">AI Coaching Advice</span>
 </div>
 <p className="text-sm text-secondary leading-relaxed italic font-medium">&quot;{lesson.ai_coaching_advice}&quot;</p>
 </div>
...
export default function LessonsPage() {
 ...
 <p className="text-xs text-primary font-semibold">{latestSessionSpecificLesson.improvement_goal}</p>
 </div>
 </div>
 </div>
 </FadeUp>
